...
本文档说明如何把平台源码包上传到一台 Linux 服务器并**快速跑起来做测试**。标注「本机」的命令在开发机执行，其余命令默认在服务器上、部署目录内执行。

> 生产级上线（PostgreSQL、gunicorn+systemd、Nginx+HTTPS、每日备份定时器）请看 **`docs/deploy-production.md`**，那里是 `./deploy/install.sh` 一步脚本 + env.sh 占位符校验的完整流程。本文档的快速测试用 SQLite + runserver，几分钟看界面。

> 部署包由源码目录打包而成，除新增的部署文档外，内容与 `main` 分支提交一一对应，可用提交号追溯。
