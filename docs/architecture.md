...
**评审人（`User.is_reviewer`）**：额外获得「评审」入口，只能看到分配给自己的送审；可下载项目书、在项目组详情页提交评审意见与决定，并可选附一份批注版项目书。项目组详情页对 staff、该组成员、以及被分配评审该组的评审人可见。
...
4. **项目书评审**：联系人上传项目书 → 提交审核、选择送审类型（决定 1~3 名评审人）并可选填说明 → 按类型随机分配评审人 → 评审人下载项目书、填写意见与决定，可选附批注版项目书 → 全部评审人均通过则方案通过并归档批注版，否则需修改后可再次提交（新一轮）。
