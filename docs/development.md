...
├── accounts/                         # 账号与成员资料
│   ├── models.py                     # User（含 must_change_password、is_reviewer、is_super_reviewer）、Profile
│   ├── roles.py                      # 身份展示口径（管理员／联系人／成员／无组）
│   ├── forms.py / views.py / urls.py / admin.py
│   ├── signals.py                    # Profile 自动创建、认证日志、登录时的待评审提醒
│   └── tests.py
...
│   ├── services.py                   # 送审与送审类型、随机分配评审人（跳过请假中）、结论汇总与批注版归档、请假登记、管理员改派、超级评审敲定
│   ├── forms.py / views.py / urls.py / admin.py
│   └── tests.py
├── equipment/                        # 设备台账与借用
...
| `reviews` | `is_reviewer` 驱动「评审」入口；送审类型决定评审人数（竞赛类 3 / 大创中期·结题 2 / 大创立项 1）并排除提交人/本组成员；可用人数不足即拒绝（提示含请假人数）；全部评审人均通过→方案通过，任一需修改→可修改后重提；批注版项目书选填，通过后按上传者归档（未上传不产生记录、重复汇总不重复归档）；批注与归档文件仅 staff/组内/被分配评审人可下载且文件名不含评审人身份；评审人身份匿名；评审人可登记请假窗口，窗口内不被抽中且 `ends_at` 一到自动恢复（无定时任务），管理员可在 Admin 列表直接改时间；手上有未完成评审时登录即提醒，成员中心顶部常驻待办卡片（计数不含请假、撤销资格者不提醒、发送用 fail_silently 以免影响登录）；同一项目组同时只允许一个未结束的轮次（上一轮未出结论时拒绝提交，已出结论则可推进下一轮）；管理员可在 Admin 详情页改派「待评审且该轮未判结论」的任务（候选与抽人同源、已完成/已判结论的记录仍只读、禁止新增与删除单条任务，但整轮可在后台删除（豁免级联检查，已归档的轮次被 PROTECT 挡住，删除写审计）、写入经服务层并审计）；超级评审（`User.is_super_reviewer`）可在「评审」看到全部进行中的轮次并一票通过/打回，不经结论汇总（否则普通评审人的「需修改」会顶回该决定），敲定时等待中的任务变为 `released`；`complete_review` 因此改为「非 pending 一律拒绝」，否则被释放者仍能提交并改写记录；可否行使由 `can_override_review()` 一处判定，页面显隐与服务层守卫共用，提交人/组成员/已在本轮持有任务者不得行使 |
