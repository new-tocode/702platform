...
│   ├── models.py                     # Board、Post、Comment（板块名大小写不敏感唯一；空板块才可删；删帖级联评论）
...
- 平台管理员问 `core.permissions.is_admin()`；只有 `is_superuser=True` 可在前端创建/删除板块。板块英文名视作原样用户内容；置顶帖排在普通帖前，各自按发布时间倒序；帖子正文与评论不翻译。
- 板块有帖子时模型外键保护与服务层行锁共同阻止删除；新帖写入也锁板块行，避免和删除并发绕过空板块条件。删帖会级联删除评论。
- 成员目录只查 `is_active=True` 账号，姓名搜索只匹配 `Profile.full_name`；空姓名或头像回退到账户名首字。`/member/profile/<id>/` 是只读资料页，公开头像、姓名/账号名、学院、专业、特长、简介、身份、个人图册、手机号和其他联系方式，不显示学号或邮箱；成员资料内容原样呈现。
...
| `discussion` | 游客与首次改密账号不可访问；作者仅改/删自己的帖，管理员可删他人帖与置顶，仅超级管理员可建/删空板块；成员目录按姓名过滤、只读资料字段白名单；发帖/删板块并发锁、删帖评论级联；英文界面翻译完整但用户内容原样 |
