...
class CompetitionRegistrationForm(forms.Form):
    group = forms.ModelChoiceField(label=_("项目组"), queryset=ProjectGroup.objects.none())
    members = forms.ModelMultipleChoiceField(
        label=_("参赛成员"),
        queryset=User.objects.none(),
        widget=forms.SelectMultiple(attrs={"size": 8}),
        help_text=_("只能选择所选项目组中的成员。"),
    )
    ...
    def clean(self):
        ...
        if not self.competition.is_registration_open:
            self.add_error(None, _("该竞赛已关闭报名或已超过报名截止时间。"))
        if group and not can_register_group(self.user, group):
            self.add_error("group", _("你无权为这个项目组登记报名。"))
        ...
        if group:
            ...
            if duplicate.exists():
                self.add_error("group", _("该项目组已经登记过这场竞赛，不能重复报名。"))
