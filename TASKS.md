...
- [x] `Profile.bio`（个人简介）字段 + 迁移
- [x] `ProfileForm` 字段顺序与排布声明（姓名｜学号、学院｜专业两列；手机号收窄并前移到特长之前；特长保持单行）
- [x] `profile.html` 左列按表单声明的排布渲染，个人简介置于最下方
- [x] `app.css` 新增两列栅格与窄栏类，窄屏回落单列
- [x] 测试 + `.po` 补译
...
- [x] `GalleryImage`（图像、排布、顺序、大小）+ 迁移
- [x] 每张 ≤5 MB（字段校验器）、每人合计 ≤100 MB（服务层行锁 + 配额）
- [x] `accounts.services`：上传、上移／下移、改排布、删除（均留审计，删图删文件）
- [x] 页面整幅宽度分区：上传、逐张的排序／排布／删除控件、用量显示
- [x] `app.css` 图册栅格（普通／大图占两格／整行），窄屏回落
- [x] 测试 + `.po` 补译
